...
首版設計目標是降低漏網廣告，不取代 YAML 規則。處理順序是先跑既有規則，只有未被明確判定為垃圾且帶有弱可疑訊號時，才進入語意相似查詢與 AI classifier。管理員、可信任成員、Bot 訊息、未授權群組與不支援訊息不會呼叫 AI。

模式行為：

- `observe`：只保存 AI 判定、語意相似訊號與偵測摘要，不建立刪除、警告、禁言或封鎖副作用。
- `delete-only`：高信心 spam 最多建立刪除候選，不累計違規階梯。
- `enforce`：AI 高信心 spam 只作為一般違規輔助訊號，不單獨觸發 critical ban。
...
- AI 與 embedding 呼叫會產生外部 provider 成本，正式環境先用 `observe` 收集誤判率。
- `max_text_chars` 會截斷送出的文字；仍應避免把秘密值放入群組訊息。
- Provider timeout、401/403、429、invalid JSON、pgvector 查詢失敗都會安全降級，既有 YAML 判定仍照常執行。
- 結構化日誌只記錄 provider、model、label、confidence、similarity、status、error_code 等摘要，不記錄完整原文。
...
SELECT
  ...
  e.created_at AT TIME ZONE 'Asia/Taipei' AS created_at
FROM message_embeddings AS q
JOIN message_embeddings AS e
  ON e.embedding_provider = q.embedding_provider
 AND e.embedding_model = q.embedding_model
 AND e.embedding_version = q.embedding_version
 AND e.dimensions = q.dimensions
WHERE q.content_fingerprint = '<content_fingerprint>'
  AND e.content_fingerprint <> q.content_fingerprint
  AND e.expires_at > now()
ORDER BY e.vector <=> q.vector
LIMIT 10;
```

語意黑名單命中可用 `semantic_blacklist_examples` 與 `semantic_blacklist_categories` 依分類查詢：
...
SELECT
  ...
  max(e.created_at) AT TIME ZONE 'Asia/Taipei' AS latest_example_at
FROM semantic_blacklist_categories AS c
LEFT JOIN semantic_blacklist_examples AS e ON e.category_id = c.id
GROUP BY c.id, c.name, c.enabled
ORDER BY latest_example_at DESC NULLS LAST;
```

批次 cluster 報表目前由 application service 產生，資料來源是 `ai_detection_events`、`semantic_manual_samples`、`message_embeddings` 的摘要欄位；報表不輸出完整原文，也不自動改寫 `configs/rules/*.yaml`。
...
- `id` 必須唯一。
- 每個規則檔可獨立使用自己的 `version`；系統會依全部規則檔內容產生規則快照版本。
- `ban` 只允許搭配 `critical`。
