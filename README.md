...
`violations` 會新增人工來源、管理員、原因及失效稽核欄位；`command_executions` 保存 `chat_id + update_id` 冪等鍵與人工操作結果。同時建立索引、約束及資料表與欄位註解。新環境不要再手動執行 `migrations/*.sql`，避免舊 SQL 約束名稱與 GORM 命名衝突。
